...
Sub addReceipts()
'04 Aug 2016 First pass at receipt data importer for Account. I want to be able to
' create the receipt info without access to Account or the DB. The inital idea is
' to create a simple email with a delimited text line for each receipt using the format used
' to paste lines between accounts in Account. Unfortunately this doesn't work as Account only
' supports pasting what it has copied. Eventually I might update Account but that isn't possible
' at present as I don't have a dev.env. for it!!
' The next idea is to paste the text into an Excel sheet using the Text Import wizard
' which appears as one of the Paste button options in the Home menu. A macro would then
' add the data to the DB in a way similar to reconciliation. This works OK however
' pasting the text copied directly from an email in Outlook doesn't cause the Import Wizard
' option to appear. The email content needs to be pasted into UltraEdit and recopied (typical
' fucking stupid MS thinking). Will need to investigate how to transfer the data from the
' email to the sheet if this is going to be the method used.
'
' For now this basically works. Still to do;
'   - needs loads more error handling
'   - should prompt for the commit (can maybe use same form as for reconciliation)
'   - the balance is not recalculated, must be done in Account
'   - make transfer from email easier
...
Dim sql As String
   ...
   Set CurrentDB = DaoDBEngine.OpenDatabase(dblocn)
   
   acc_id = getAccountID(CurrentDB, failreason)
   
   If acc_id > -1 Then
      sql = "select * from transaction where AccountId=" & acc_id & " order by sequence asc"
      ...
      Do While .Cells(rownum, 1).Text <> ""
         tdate = .Cells(rownum, 1).Text
         ttype = .Cells(rownum, 2).Text
         tdesc = .Cells(rownum, 3).Text
         tdebit = .Cells(rownum, 4).Text
         amtv = Val(tdebit)
         rs.AddNew
         rs("debit") = amtv
         rs("AccountId") = acc_id
         rs("Date") = tdate
         rs("comment") = Left$(tdesc, rs("comment").Size)
         rs("type") = ttype
...
Dim failreason As String
   ...
   If acc_id > -1 Then
      checkStatement CurrentDB, acc_id
   Else
      MsgBox "No matching account found in database." & vbCrLf & failreason
   End If
...
Dim actionstr As String
   ...
   sql = "select * from transaction where AccountId=" & accid & " and checked=false order by sequence asc"
   DBEngine.BeginTrans
   ...
   With ActiveSheet
      ...
      Do While .Cells(rownum, 1).Text <> ""
         ...
         If amtvalue < 0 Then
            debit = amtvalue * -1#
            transsql = "credit is null and debit = " & debit
         Else
            credit = amtvalue
            transsql = "debit is null and credit = " & credit
         End If
         ...
         transsql = transsql & " and checked=false"
         ...
         If rs.NoMatch Then
            ...
            If notrcncld = ALLOK Then
               ...
               .Cells(rownum, COL_DBSEQ) = rs("sequence")
         ...
         Else
            ...
            'Debug.Print amtdate
            Do While (Abs(DateDiff("d", rs("Date"), amtdate)) > 3) And Not rs.NoMatch
...
Dim i As Integer
   ...
   msg = msg & "Date:    " & arow.Columns(COL_DATE) & "(Statement) " & rs("date") & "(Database)" & vbCrLf
...
amt = Trim$(arow.Columns(COL_VALUE))
   ...
   End If
   rs("AccountId") = accid
   rs("stid") = arow.Columns(COL_STATNUM)
   rs("checked") = True
   rs("Date") = arow.Columns(COL_DATE)
...
Dim stmntid As String
   ...
   If "" & rs("stid") = "" Then
      ...
      rs("checked") = True
      rs.Update
   ElseIf rs("stid") = stmntid Then
      rs.Edit
      rs("checked") = True
      rs.Update
   Else
      reconcileTransaction = "Statement id '" & rs("stid") & "' already present"
   End If
End Function
...
Dim accid As Long
   ...
   sql = "select acc_id, acc_code, acc_curr from account"
   Set rs = db.OpenRecordset(sql)
   rs.MoveFirst
   accid = -1
   
   Do While Not rs.EOF
      normacc = StrRepl(rs("acc_code"), "-", "")
      normacc = StrRepl(normacc, " ", "")
      Debug.Print rs("acc_id"), normacc, rs("acc_code"), rs("acc_curr")
      ' Duplicate account codes exist from the pre-EURO days
      If normacc = sheetacc Then
         If sheetcur = rs("acc_curr") Then
            accid = rs("acc_id")
            Exit Do
         End If
      End If
